...
from niypep.interfaces.ants.resampling import ApplyTransforms
...
def anatomical_workflow(
    ...
    Outputs:
        ...
        normalization_transform (file): transform matrix to go from T1 to template space
    ...
    normalization = Node(Registration(from_file=ants_normalization_settings),name='normalization')
    normalization.inputs.num_threads = num_threads
    ...
    anatomical_wf.connect([
        ...
        [('composite_transform','normalization_transform'),
         ('warped_image','normalized_brain')])
    ])

    #Define Segmented Tissue Normalization Node
    tissue_transform = Node(ApplyTransforms(from_file=ants_transform_settings),name='tissue_transform')
    ...
    anatomical_wf.connect([
        ...
        ('inputnode','tissue_transform',[('template_image','reference_image')]),('tissue_transform','outputnode',[('output_image','normalized_segmentation')])
    ])
